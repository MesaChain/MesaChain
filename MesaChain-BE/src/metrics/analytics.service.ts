...
export class AnalyticsService {
  ...
  async getAnalytics(period: AggregationPeriod = AggregationPeriod.DAILY): Promise<AnalyticsResult> {
  ...
  private async generateAnalytics(period: AggregationPeriod): Promise<AnalyticsResult> {
  ...
  private async generateSummary() {
    ...
    const categoriesCount = await this.prisma.metric.groupBy({
      by: ['category'],
    ...
    const averageValue = await this.prisma.metric.aggregate({
      _avg: { value: true },
  ...
  private async generateTrendAnalysis(period: AggregationPeriod): Promise<Record<string, TrendAnalysis>> {
  ...
  private calculateConfidence(values: number[], slope: number, intercept: number): number {
    ...
    return Math.max(0, Math.min(1, 1 - (meanError / meanValue)));
  }

  private generateInsights(summary: any, trends: Record<string, TrendAnalysis>): string[] {
    const insights: string[] = [];
    
    // Category insights
    const topCategory = Object.entries(summary.categoriesCount)
      .sort(([,a], [,b]) => (b as number) - (a as number))[0];
    if (topCategory) {
      insights.push(`${topCategory[0]} metrics represent the largest category with ${topCategory[1]} entries`);
    }
    
    // Trend insights
    const increasingTrends = Object.entries(trends)
      .filter(([, trend]) => trend.trend === 'increasing').length;
    const decreasingTrends = Object.entries(trends)
      .filter(([, trend]) => trend.trend === 'decreasing').length;
    
    if (increasingTrends > decreasingTrends) {
      insights.push(`Overall positive trend detected: ${increasingTrends} metrics showing growth`);
    } else if (decreasingTrends > increasingTrends) {
      insights.push(`Attention needed: ${decreasingTrends} metrics showing decline`);
    }
    
    return insights;
  }

  private generateRecommendations(summary: any, trends: Record<string, TrendAnalysis>): string[] {
    const recommendations: string[] = [];
    
    // Performance recommendations
    const strongDeclines = Object.entries(trends)
      .filter(([, trend]) => trend.trend === 'decreasing' && trend.changePercentage < -20);
    
    if (strongDeclines.length > 0) {
      recommendations.push(`Investigate declining metrics: ${strongDeclines.map(([name]) => name).join(', ')}`);
    }
    
    // Data collection recommendations
    if (summary.totalMetrics < 1000) {
      recommendations.push('Consider increasing data collection frequency for better insights');
    }
    
    return recommendations;
  }

  async getPredictiveAnalytics(metricName: string, days: number = 30): Promise<any> {
    const cacheKey = this.cacheService.generateCacheKey('predictive', metricName, days.toString());
